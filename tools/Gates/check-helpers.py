...
RATCHET_REASON = ("Per-mod ceiling on COPIED helpers (identical body, second and later "
    "copies). Lower is free; raising it means you pasted a helper - promote "
    "it instead. See check-helpers.py.")
BASE_FILE = os.path.join(HERE, "helper-baseline.json")

# Two definition shapes, because only the first was ever matched. A sweep of
# Dragonfly found `function DFDeck.saveSizes` and `function DFPlayerDeck.saveSizes`
# byte-identical and INVISIBLE to this checker - module-level functions are how
# a mod's public surface is written, so the duplication that matters most for
# promotion to shared/ was precisely the duplication being missed.
# The captured name is the LAST segment, so DFDeck.saveSizes and
# DFPlayerDeck.saveSizes collide on "saveSizes" - and the body comparison
# ignores the name anyway, so a renamed copy still cannot hide.
DEF = re.compile(r"^\s*(?:local\s+)?function\s+(?:[A-Za-z_][A-Za-z0-9_]*[.:])*([A-Za-z_][A-Za-z0-9_]*)\s*\(")
...
# Smallest definition worth calling a copied helper. See the note at the yield.
MIN_BODY_LINES = 2
BLOCK = re.compile(r"\b(function|for|while|repeat|if|end|until)\b")


def helpers(path):
    """Yield (name, line, normalized_body, raw_line_count) per function definition."""
    text = read(path)
    ...
    for i, line in enumerate(lines):
        ...
        # Normalise BOTH shapes to one token so a local copy and a module-level
        # copy of the same body still compare equal.
        norm = re.sub(r"^(?:local )?function [A-Za-z_][A-Za-z0-9_.:]*", "function _", norm)
        # A whole function on ONE line is a stub, not a helper: `function
        # X:onMouseUp(a, b) endDrag(self) end`, or a one-line field return.
        # When module-level definitions became visible, 14 of 61 groups were
        # exactly this - ISUI event overrides that every panel is REQUIRED to
        # declare and that nobody can promote anywhere. They are boilerplate the
        # engine asks for, not duplication a shared/ file could absorb, and at
        # a quarter of the list they were drowning the real signal.
        if end - i + 1 >= MIN_BODY_LINES:
