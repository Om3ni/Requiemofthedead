...
Events.OnServerStarted.Add = function(f) serverStarted = f end
...
-- RDMeter.lua:324 flushes on arming so the wire directory exists the moment the
-- armed line prints and the two cannot disagree about whether the probe is on.
flush = function() end }

local onlinePlayers = { size = function() return 2 end }
getOnlinePlayers = function() return onlinePlayers end
ModData = {
    getOrCreate = function() return { marker = "meter" } end,
    transmit = function(tag) return tag end,
}
...
if type(serverStarted) ~= "function" then print("FATAL: RDMeter registered no server-start hook"); os.exit(2) end
serverStarted()
tickHandlers[1]()
...
ModData.transmit("MeterFixture")
local initial = dumpRows()
local mdata = rowFor(initial.rows, "MDATA", "ModData:MeterFixture")
isTrue("post-start ModData meter records one transmission", mdata and mdata.calls == 1, mdata)
isTrue("post-start ModData meter multiplies by the live recipient count", mdata and mdata.est > 2, mdata)
