...
-- ---------------------------------------------------------------------------
-- Degradation. A missing text manager must not take the panel down with it.
-- ---------------------------------------------------------------------------
local savedTM = getTextManager
getTextManager = function() error("no text manager this early") end
ok("recompute survives a dead text manager", pcall(DFMetrics.recompute))
eq("...and leaves spacing at its tuned values", DFKit.metrics.pad, 8)
getTextManager = savedTM

-- THE POISONED REFERENCE. DFMetrics.recompute() runs at file load, which in PZ
-- can be before the text manager exists. If that failed measurement were cached
-- as the reference, a real font of 18 against a remembered 15 would be a
-- permanent x1.2 on every panel at DEFAULT text size - a visual change this
-- step is explicitly not allowed to make. Simulated by loading the module in a
-- world with no text manager at all, then giving it one.
local DFMETRICS_SAVED = DFMetrics
DFKit.metrics = { pad = 8, gap = 6, btnH = 24, rowH = 22, headerH = 20, radius = 3 }
DFMetrics = nil
getTextManager = function() error("not ready") end
dofile(SRC)                              -- load with nothing measurable
getTextManager = savedTM                 -- ...then the game finishes booting
H.small = 18                             -- and the real font is NOT the fallback
DFMetrics.recompute()
eq("a failed first measure is not cached as the reference", DFKit.metrics.scale, 1)
eq("...so default text size still gets the tuned value", DFKit.metrics.pad, 8)
H.small = 15
DFMetrics = DFMETRICS_SAVED
